...
def genetic_search(problem, ngen=1000, pmut=0.1, n=20):
    ...
    numb = len(s)
    states = []
    # generate population by randomizing the positions of 0 through n-1
    for i in range(n):
        temp = [*range(0, numb, 1)]
        #random.shuffle(temp)
        for j in range(numb):
            temp[j] = (random.randrange(numb))
        states.append(tuple(temp))
    # states = [problem.result(s, a) for a in problem.actions(s)]
    # print(states)
    gene_pool = tuple([*range(0, numb, 1)])
    f_thres = 0
    return genetic_algorithm(states[:n], problem.value, gene_pool, f_thres, ngen, pmut)


def genetic_algorithm(population, fitness_fn, gene_pool=[0, 1], f_thres=None, ngen=1000, pmut=0.1):
    """[Figure 4.8]"""
    global_max = population[0]
    for i in range(ngen):
        print(i, "/", ngen)
        population = [mutate(recombine(*select(2, population, fitness_fn)), gene_pool, pmut)
                      for i in range(len(population))]

        local_max = min(population, key=fitness_fn)
        if ( fitness_fn(local_max) < fitness_fn(global_max) ):
...
def select(r, population, fitness_fn):
    ...
    # print(list(fitnesses)) crashes the program for some reason
...
def mutate(x, gene_pool, pmut):
    ...
    return list(x[:c]) + [new_gene] + list(x[c + 1:])
...
class NQueensProblem(Problem):
    ...
    def value(self, state):
        # the value of a state is the heuristic?
        return self.gen_h(state)

    def gen_h(self, state):
